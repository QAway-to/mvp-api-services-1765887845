...
async function handleOrderUpdated(order) {
  ...
  // ✅ MICROSTEP A2.2: Anti-loop guard - check provenance marker
  try {
    ...
    if (hasMarker && provenanceResult.value) {
      ...
      if (markerTs) {
        ...
        // Check if marker is from Bitrix and within TTL
        if (markerValue.source === 'bitrix' && 
    ...
    if (shouldSkip) {
      ...
      // Skip Bitrix update - this is a self-triggered update from Bitrix→Shopify fulfillment
      return;
    }
